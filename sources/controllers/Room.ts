...
class Room extends Controller {
    ...
    async getOne(idRoom) : Promise<Result> {
        try {
    ...
    async insert(pRoom) : Promise<Result> {
    ...
    async delete(idRoom) : Promise<Result> {
        try {
            let smartobjectsRequest = await this.sqlSmartobject.getAllByField({ room: idRoom })
            if (smartobjectsRequest.error) {
                return smartobjectsRequest
    ...
    async insertRoomProfile(idRoom, idProfile) : Promise<Result> {
        try {
    ...
    async deleteRoomProfile(idRoom, idProfile) : Promise<Result> {
        try {
            let roomRequest = await this.sqlRoom.getOne(idRoom)
            if (roomRequest.error) {
                return roomRequest
    ...
    /* Déprécié */
    async executeAction(idRoom, idAction, settings) : Promise<Result> {
        ...
        actions.forEach(async (action) => {
            await this.smartobjectController.executeAction(action.id, action.action.id, settings)
        })
