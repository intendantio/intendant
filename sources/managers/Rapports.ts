...
const toadSchedulerModule = require('toad-scheduler')
...
class Rapport extends Manager {
    ...
    constructor(core: Core) {
        ...
        this.scheduler = new toadSchedulerModule.ToadScheduler()
    ...
    async before() {
        ...
        getAllRapports.data.forEach(rapport => {
            if (this.instances.has(rapport.id) == false) {
                ...
                let task = new toadSchedulerModule.Task(rapport.id.toString(), async () => {
                    this.executeAction(rapport)
                })
                this.executeAction(rapport)

                this.scheduler.addSimpleIntervalJob(
                    new toadSchedulerModule.SimpleIntervalJob({ seconds: rapport.interval }, task, "intendant_rapport_" + rapport.id)
                )
