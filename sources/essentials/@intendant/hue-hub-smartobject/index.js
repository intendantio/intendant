...
class HueHub extends SmartObject {
    ...
    async refreshToken() {
        this.tracing.verbose(Package.name, "Token is expired, access_token must be refresh")
    ...
    async getDevice(settings = {}) {
        ...
        if (resultCache.cache) {
            this.tracing.verbose(Package.name, "Use cache : " + Package.name + "-device-" + this.id)
            return {
                error: false,
                package: Package.name,
                message: "",
        ...
        try {
            ...
            this.tracing.error(Package.name, error.toString())
            return {
                error: true,
                package: Package.name,
    ...
    async getStatus() {
        ...
        if (resultCache.error) { return resultCache }
        if (resultCache.cache) {
            this.tracing.verbose(Package.name, "Use cache " + Package.name + "-state-" + this.id)
            return {
                error: false,
                package: Package.name,
                message: "",
