...
class NetatmoIndoorCamera extends SmartObject {
    ...
    private CACHE_ACCESS_TOKEN: String
    private CACHE_INDOOR_CAMERA_SELECTED: String
    private CACHE_HOME_SELECTED: String

    private device: any
    private home: any

    private detectors: Array<any>
    private dataSources: Array<Object>
    private widgets: Array<Object>
    private triggers: Array<Object>
    ...
    async prepare() {
        try {
            ...
            if (resultHomeCache.error) {
                ...
                Tracing.verbose(Package.name, "Step 1 - Select home")
                //Step 1 Select Home
                ...
                if (homes.length == 0) {
                    ...
                    Tracing.warning(Package.name, "Not find home")
                    return
                } else {
                    this.actions = [
                        {
                ...
                Tracing.verbose(Package.name, "Step 1 - Select Indoor Device")
                ...
                if (resultDeviceCache.error) {
                    ...
                    let resultHome : any = this.getHome(resultHomesData.data, resultHomeCache.data)
                    if (resultHome.error) {
                        return resultHome
                    }
                    ...
                    if (devices.length == 0) {
                        Tracing.warning(Package.name, "Not find camera at this home")
                        this.state = {
                            status: "ERROR",
                    ...
                    Tracing.verbose(Package.name, "Step 3 - Execute")
                    ...
                    let resultDetectors : any = this.getDetectors(resultHome.data, resultHome.data, resultDeviceCache.data)
            ...
            Tracing.error(Package.name, "Error occured")
        }
    ...
    getHome(homeData, idHome) {
        ...
        if (currentHome == false) {
            Tracing.warning(Package.name, "Home with id " + idHome + " not found, please select new home")
            this.core.controller.storage.removeItem(this.CACHE_HOME_SELECTED)
            return {
                package: Package.name,
                error: true,
    ...
    getCamera(home, idCamera) {
        ...
        if (currentCamera == false) {
            Tracing.warning(Package.name, "Camera with id " + idCamera + " not found, please select new home")
            this.core.controller.storage.removeItem(this.CACHE_INDOOR_CAMERA_SELECTED)
            return {
                package: Package.name,
                error: true,
    ...
    */
    ...
    async __selectHome(settings : any = {}) {
    ...
    async __selectDevice(settings : any = {}) {
    ...
    async __event(settings : any = {}) {
        if (this.home.id == settings.home_id) {
            if (this.device.id == settings.device_id) {
                ...
                if (detector) {
                    Tracing.verbose(Package.name, "Event on " + detector.name + " in " + settings.home_name + " (detector)")
                    this.executeCallback("event_" + settings.event_type + "_" + detector.id, settings)
                } else {
                    Tracing.verbose(Package.name, "Event on " + settings.home_name + " (camera)")
    ...
    async __getContext() {
        let resultCurrentHome : any = await this.getCurrentHome()
        if (resultCurrentHome.error) {
            return resultCurrentHome
        }
        let resultCurrentCamera : any = await this.getCurrentCamera()
